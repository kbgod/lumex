...
func TestContext_SetParseMode(t *testing.T) {
	t.Run("empty send message opts", func(t *testing.T) {
		...
		cl.On(
			...
			mock.MatchedBy(
				func(req any) bool {
					r, _ := req.(*lumex.SendMessageRequest)
					if r == nil {
						return false
					}
					return r.ChatID == 1 &&
		...
		bot, err := lumex.NewBot(fakeToken, lumex.WithClient(cl), lumex.WithoutTokenCheck())
		...
		cl.On(
			...
			mock.MatchedBy(
				func(req any) bool {
					...
					return r.ChatID == 1 &&
						r.Text == "test" &&
						r.ParseMode == lumex.ParseModeHTML
				},
			),
		).Return(
			json.RawMessage(`{"message_id":123,"chat":{"id":1},"text":"test"}`),
...
func TestContext_Reply(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				return r.ChatID == 1 &&
					r.Text == "test" &&
					r.ReplyParameters.MessageID == 225
			},
		),
	).Return(
	...
	bot, err := lumex.NewBot(fakeToken, lumex.WithClient(cl))
...
func TestContext_ReplyVoid(t *testing.T) {
		...
		cl.On(
			...
			mock.MatchedBy(
				func(req any) bool {
					...
					return r.ChatID == 1 &&
						r.Text == "test" &&
						r.ParseMode == lumex.ParseModeMarkdown
				},
			),
		).Return(
			json.RawMessage(`{"message_id":123,"chat":{"id":1},"text":"test"}`),
	...
	t.Run("invalid token", func(t *testing.T) {
		...
		bot, err := lumex.NewBot(invalidToken, lumex.WithClient(cl), lumex.WithoutTokenCheck())
		assert.NoErrorf(t, err, "lumex.NewBot() = %v; want <nil>", err)
		...
		cl.On(
			...
			mock.MatchedBy(
				func(req any) bool {
					...
					keyboard, ok := r.ReplyMarkup.(*lumex.ReplyKeyboardMarkup)
					return r.ChatID == 1 &&
						r.Text == "test" &&
						ok &&
		...
		cl.On(
			...
			mock.MatchedBy(
				func(req any) bool {
					...
					keyboard, ok := r.ReplyMarkup.(*lumex.InlineKeyboardMarkup)
					return r.ChatID == 1 &&
						r.Text == "test" &&
						ok &&
...
func TestContext_ReplyPhoto(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.SendPhotoRequest)
				if r == nil {
					return false
				}
				ok := r.Photo != nil
				hasParseMode := r.ParseMode != ""
				hasCaption := r.Caption != ""

				return r.ChatID == 1 && ok && !hasParseMode && !hasCaption
			},
		),
	).Return(
	...
	testPhotoFileID := lumex.InputFileID("test_photo")
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				capVal := r.Caption
				return r.ChatID == 1 && capVal == "success"
			},
		),
	).Return(
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				capVal := r.Caption
				return r.ChatID == 1 && capVal == "error"
			},
		),
	).Return(
		nil,
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				hasMarkup := r.ReplyMarkup != nil
				hasCaption := r.Caption != ""
				return r.ChatID == 1 && hasMarkup && !hasCaption
			},
		),
	).Return(
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				hasMarkup := r.ReplyMarkup != nil
				capVal, hasCaption := r.Caption, r.Caption != ""
				return r.ChatID == 1 && hasMarkup && hasCaption && capVal == "with_menu"
			},
		),
	).Return(
...
func TestContext_ReplyPhotoWithMenuVoid(t *testing.T) {
	...
	bot, _ := lumex.NewBot("123:test", lumex.WithClient(cl))
...
func TestContext_ReplyVideo(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.SendVideoRequest)
				if r == nil {
					return false
				}
				ok := r.Video != nil
	...
	testVideoFileID := lumex.InputFileID("test_video")
...
func TestContext_Answer(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.AnswerCallbackQueryRequest)
				if r == nil {
					return false
				}
				hasText := r.Text != ""
				return r.CallbackQueryID == "cb_123" && !hasText
			},
		),
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				textVal, hasText := r.Text, r.Text != ""
				return r.CallbackQueryID == "cb_123" && hasText && textVal == "hello"
			},
		),
	).Return(json.RawMessage(`true`), nil).Once()
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				textVal, hasText := r.Text, r.Text != ""
				showAlert, hasShowAlert := r.ShowAlert, r.ShowAlert
				return r.CallbackQueryID == "cb_123" && hasText && textVal == "hello" && hasShowAlert && showAlert
			},
		),
	).Return(json.RawMessage(`true`), nil).Once()
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				textVal, hasText := r.Text, r.Text != ""
				showAlert, hasShowAlert := r.ShowAlert, r.ShowAlert
				return r.CallbackQueryID == "cb_123" && hasText && textVal == "alert1" && hasShowAlert && showAlert
			},
		),
	).Return(json.RawMessage(`true`), nil).Once()
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				textVal, hasText := r.Text, r.Text != ""
				showAlert, hasShowAlert := r.ShowAlert, r.ShowAlert
				return r.CallbackQueryID == "cb_123" && hasText && textVal == "alert2" && hasShowAlert && showAlert
			},
		),
	).Return(json.RawMessage(`true`), nil).Once()
...
func TestContext_AnswerQuery(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.AnswerInlineQueryRequest)
				if r == nil {
					return false
				}
				idVal, ok := r.InlineQueryID, r.InlineQueryID != ""
				hasCacheTime := r.CacheTime != 0
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				idVal, ok := r.InlineQueryID, r.InlineQueryID != ""
				cacheVal, hasCacheTime := r.CacheTime, r.CacheTime != 0
...
func TestContext_DeleteMessage(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.DeleteMessageRequest)
				if r == nil {
					return false
				}
				chatIdVal, hasChatId := r.ChatID, r.ChatID != 0
				msgIdVal, hasMsgId := r.MessageID, r.MessageID != 0
...
func TestContext_EditMessageText(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				r, _ := req.(*lumex.EditMessageTextRequest)
				if r == nil {
					return false
				}
				return fmt.Sprintf("%v", r.ChatID) == "1" &&
					fmt.Sprintf("%v", r.MessageID) == "42" &&
					fmt.Sprintf("%v", r.Text) == "basic_text" &&
					r.ParseMode == ""
			},
		),
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				pm := r.ParseMode
				return fmt.Sprintf("%v", r.Text) == "opts_text" && pm == "HTML"
			},
		),
	...
	cl.On(
		...
		mock.MatchedBy(
			func(req any) bool {
				...
				pm := r.ParseMode
				return fmt.Sprintf("%v", r.Text) == "ctx_pm_text" && pm == "Markdown"
			},
		),
...
func TestContext_ReplyEmojiReaction(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(func(req any) bool {
			r, _ := req.(*lumex.SetMessageReactionRequest)
			if r == nil {
				return false
			}
			chatOK := fmt.Sprintf("%v", r.ChatID) == "1"
			msgOK := fmt.Sprintf("%v", r.MessageID) == "42"
			reactions, ok := r.Reaction, r.Reaction != nil
			isBig, hasIsBig := r.IsBig, r.IsBig

			return chatOK && msgOK && ok && len(reactions) == 1 && (!hasIsBig || isBig == false)
		}),
	).Return(json.RawMessage(`true`), nil).Once()
	...
	cl.On(
		...
		mock.MatchedBy(func(req any) bool {
			...
			return chatOK && msgOK && ok && len(reactions) == 2
		}),
	).Return(json.RawMessage(`true`), nil).Once()
...
func TestContext_ReplyEmojiBigReaction(t *testing.T) {
	...
	cl.On(
		...
		mock.MatchedBy(func(req any) bool {
			...
			return chatOK && msgOK && ok && len(reactions) == 1 && hasIsBig && isBig
		}),
	).Return(json.RawMessage(`true`), nil).Once()
	...
	cl.On(
		...
		mock.MatchedBy(func(req any) bool {
			...
			return chatOK && msgOK && ok && len(reactions) == 2 && hasIsBig && isBig
		}),
	).Return(json.RawMessage(`true`), nil).Once()
