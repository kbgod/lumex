...
- **omitempty**: optional fields get `,omitempty`. Optional pointer/slice/interface/enum/scalar all drop cleanly; that's WHY optional `*InputFile`/`int64` are chosen over value structs (a value struct with omitempty always serialises). **Exception `omitOption` (render.go): `allowed_updates` (getUpdates/setWebhook) uses `,omitzero` instead** — Telegram treats an empty list `[]` ("all update types except a few") differently from an absent field ("keep previous setting"), so an empty non-nil `[]string{}` must serialise as `[]`; `omitempty` would drop it, `omitzero` (Go 1.24+) drops only `nil`. Covered by `TestOmitOption`.
